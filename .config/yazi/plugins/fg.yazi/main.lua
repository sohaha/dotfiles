...
function M:entry(job)
	...
	elseif args[1] == "fzf" then
		cmd_args = [[fzf --preview="bat --color=always {}"]]
	elseif args[1] == "fzfc" then
		cmd_args = [[find . -type f | fzf --preview="bat --color=always {}"]]
	elseif args[1] == "fzfd" then
		cmd_args = [[find . -type d \( \
			-name ".git" -o \
			-name "node_modules" -o \
			-name ".cache" -o \
			-name ".history" -o \
			...
			-name ".next" -o \
			-name ".nuxt" \
		\) -prune -o -type d -print | fzf --preview="bat --color=always {}"]]
